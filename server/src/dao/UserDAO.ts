...
export class UserDAO {
  ...
  //them moi user
  async create(data: { user_name: string; email: string; password_hash: string; full_name?: string }): Promise<number> {
    const [result] = await pool.query<ResultSetHeader>(
      'INSERT INTO Users (user_name, email, password_hash, full_name, role) VALUES (?, ?, ?, ?, ?)',
      [data.user_name, data.email, data.password_hash, data.full_name || null, 'student']
    );
  ...
  //lau ds tat ca user
  async findAll(page: number = 1, limit: number = 20): Promise<{ users: UserRow[]; total: number }> {
