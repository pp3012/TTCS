...
export default function StatisticsPage() {
  ...
  useEffect(() => { subjectApi.getAll().then(r => { const s = r.data.data || []; setSubjects(s); if (s.length > 0) setSelectedSubject(s[0].subject_id); }); }, []);
  ...
  const scoreHistory = stats?.score_history?.map(s => ({
    time: new Date(s.submit_time).toLocaleDateString('vi-VN'), score: s.score,
  })) || [];
